...
<div class="fixed bottom-5 left-5 z-40" data-theme-switcher>
    {#if showMenu}
        <div
        ...
        >
            ...
            {#each themes as theme}
                <button
                    ...
                    class="w-full px-3 py-2.5 flex items-center gap-3 rounded-xl text-sm text-left transition-colors duration-200 hover:bg-zinc-50 dark:hover:bg-white/5 cursor-pointer {currentTheme ===
                    theme.name
    ...
    <button
        onclick={toggleMenu}
        class="w-11 h-11 rounded-full bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl ring-1 ring-black/6 dark:ring-white/10 shadow-lg shadow-black/4 flex items-center justify-center transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] hover:scale-105 cursor-pointer {showMenu
            ? 'rotate-180'
            : ''}"
    >
